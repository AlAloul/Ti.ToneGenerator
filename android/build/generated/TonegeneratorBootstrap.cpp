...
#define TAG "de.appwerft.tongenerator"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tonegenerator_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tonegenerator.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TonegeneratorBindings::lookupGeneratedInit(
...
static void Tonegenerator_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tonegenerator_getBinding)->GetFunction());
}

static void Tonegenerator_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TonegeneratorBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TonegeneratorBinding = {
	"de.appwerft.tongenerator",
	Tonegenerator_init,
	Tonegenerator_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_de_appwerft_tongenerator_TonegeneratorBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.tongenerator", &TonegeneratorBinding);
	titanium::KrollBindings::addExternalLookup(&(::TonegeneratorBindings::lookupGeneratedInit));
}
